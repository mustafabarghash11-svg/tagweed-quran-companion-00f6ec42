...
import { ChevronRight, Loader2, AlertCircle, Copy, Share2, Heart, BookOpen } from 'lucide-react';
import { Button } from '@/components/ui/button';
import { toast } from 'sonner';
import { getHadithById, Hadith } from '@/data/hadithData';
...
export default function HadithViewer() {
  ...
  useEffect(() => {
    if (id) {
      setTimeout(() => {
        const found = getHadithById(parseInt(id));
        setHadith(found || null);
        setLoading(false);
      }, 100);
    }
  }, [id]);
  ...
  if (!hadith) {
    return (
      <div className="flex min-h-screen flex-col items-center justify-center gap-4" dir="rtl">
        ...
        <p className="font-ui text-muted-foreground">الحديث غير موجود</p>
        <Link to="/hadith">
        ...
        <div className="rounded-2xl border border-primary/20 bg-card p-5 space-y-4">
          <div className="flex items-center justify-between flex-wrap gap-2 pb-3 border-b border-primary/10">
            <span className="rounded-full bg-primary/10 px-3 py-1 font-ui text-sm font-bold text-primary">
              {hadith.collection} • رقم {hadith.hadithNumber}
            </span>
            {hadith.grade && (
          ...
          <div className="pt-3 border-t border-primary/10">
            <p className="font-ui text-xs text-muted-foreground">
              <span className="font-bold">الكتاب:</span> {hadith.book} • <span className="font-bold">الباب:</span> {hadith.chapter}
            </p>
          </div>
